...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    newGrid.assign(rows,vector < float>(cols,0));

	float center_p = 1 - blurring;
	float corner_p = blurring/12;
	float adjacent_p = blurring/6;
	float window[3][3]={
	...
	for (int i =0 ;i<rows;i++)
	{
		for (int j=0;j<cols;j++)
		{
			for (int dx=-1;dx<2;dx++)
			{
				...
				{
					float mul = window[1+dx][1+dy];
					int new_x = (j+dx+rows)%cols;
					int new_y = (i+dy+cols)%rows;
					float new_pro = mul * grid[i][j];
                    newGrid[new_y][new_x] += new_pro;
				}
			}
		}
	}
